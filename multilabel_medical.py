...
parser = argparse.ArgumentParser(description="ranking with medical dataset")
parser.add_argument(
    "--slots_per_label",
    type=int,
    default=30
...
graph_dir = f'graph{os.sep}multilabel-graph{os.sep}medical'

dataset = MedicalDataset(slots_per_label=args.slots_per_label)
...
match_ranking        = match_ranker.rank_lazy()
...
match_rank_test_score, match_rank_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    match_ranking)
TR_test_score, TR_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    TR_ranking)
NTR_test_score, NTR_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    NTR_ranking)
PR_and_test_score, PR_and_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    PR_and_ranking)
PR_or_test_score, PR_or_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    PR_or_ranking)
random_test_score, random_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(
    random_ranking)
...
plot_slot_progress(axes[3], PR_and_match_group_comp, PR_AND_rank_name)
plot_slot_progress(axes[4], PR_or_match_group_comp, PR_OR_rank_name)
plot_slot_progress(axes[1], TR_match_group_comp, TR_rank_name)
plot_slot_progress(axes[2], NTR_match_group_comp, NTR_rank_name)
plot_slot_progress(axes[5], random_match_group_comp, random_rank_name)
axes[5].legend()
fig.supylabel("Percentage of Filled Slots", fontsize=12)
fig.suptitle(
    f"Medical Dataset ({args.slots_per_label} Slots Per Label)", fontsize=14)
fig.tight_layout()
