...
parser = argparse.ArgumentParser(description="ranking with delicious dataset")
parser.add_argument(
    "--slots_per_group",
    type=int,
    default=30
...
graph_dir = f'graph{os.sep}multilabel-graph{os.sep}delicious'

dataset = DeliciousDataset(slots_per_group=args.slots_per_group)
...
print(f"selected_label names: {[dataset.labels[i][0] for i in dataset.selected_label_indices]}")
print()
...
E = np.sum(dataset.test_U_samples, axis=(1, 2))
...
print(f"ground truth labels percentage: {100 * np.sum(dataset.test_Y_selected, axis=0) / dataset.test_samples}") 
...
match_rank_test_score, match_rank_match_group_comp = dataset.compute_ground_truth_matching_score_and_matched_group_number(match_ranking)
TR_test_score, TR_match_group_comp                 = dataset.compute_ground_truth_matching_score_and_matched_group_number(TR_ranking)
NTR_test_score, NTR_match_group_comp               = dataset.compute_ground_truth_matching_score_and_matched_group_number(NTR_ranking)
PR_and_test_score, PR_and_match_group_comp         = dataset.compute_ground_truth_matching_score_and_matched_group_number(PR_and_ranking)
PR_or_test_score, PR_or_match_group_comp           = dataset.compute_ground_truth_matching_score_and_matched_group_number(PR_or_ranking)
random_test_score, random_match_group_comp         = dataset.compute_ground_truth_matching_score_and_matched_group_number(random_ranking)
...
def plot_slot_progress(axes, match_group_comp, ranking_name):
    match_abs_num = match_group_comp.T * dataset.test_slots.reshape((len(dataset.test_slots), 1)) / 100
    for g, g_line in enumerate(match_abs_num):
        axes.plot(np.arange(1, len(g_line) + 1), 100 * g_line / dataset.test_slots[g], label=f"g{g + 1}", alpha=0.7, linewidth=1)
    threshold = threshold_that_satisfies_slot_constraint(match_abs_num) + 1
    if threshold != np.inf:
        axes.axvline(x=threshold, color='black', linestyle='dashdot')
        axes.set_xlabel(f"min |C|={threshold} ({(threshold / np.sum(dataset.test_slots)).item():.2f} |slots|)", fontsize=8)
...
fig, axes = plt.subplots(1, 6, sharex=True, sharey=True, figsize=(14, 4), dpi=400, gridspec_kw=dict(wspace=0.2, left=0.05))
...
fig.suptitle(f"Delicious Dataset ({args.slots_per_group} Slots Per Label)", fontsize=14)
fig.tight_layout()
