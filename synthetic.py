...
basic_name = f"elibility-{args.eligibility}-groups-{args.groups}-slots-{args.slots}-r-{args.r}-magnitude-{args.magnitude}"


class SyntheticExp(GeneralExperiment):
    def __init__(self, student_num, slot_list, group_num, assigned_group, magnitude, train_r, test_r) -> None:
        super().__init__()
        self.r = train_r + test_r
        self.slot_total_number = slot_list.sum()
        self.slot_list = slot_list

        group_assignment_obj = BinomialGroupAssignment(student_num, group_num, assigned_group)
        self.group_assignment = group_assignment_obj.assignment
        self.applicant_list = group_assignment_obj.student_list
        self.student_num = group_assignment_obj.student_num
        ...
        def pr_assigner(i):
            ...
            pr = {
                'small' : 0.2 + (0.03 * np.arange(group_num))[self.student_group_matrix[i].nonzero()[0]],
                'medium': 0.3 + (0.03 * np.arange(group_num))[self.student_group_matrix[i].nonzero()[0]],
                'large' : 0.4 + (0.03 * np.arange(group_num))[self.student_group_matrix[i].nonzero()[0]],
            }[magnitude]
            if assigned_group != 1:
                ret_q = np.zeros((assigned_group,))
                for j in range(assigned_group):
                    ret_q[j] = np.clip(pr[j] + noise_maker(), 1e-4, 1 - 1e-4)
                return ret_q
            else:
                return np.clip(pr + noise_maker(), 1e-4, 1 - 1e-4)

        self.pr_assignment = np.zeros((student_num,)) if assigned_group == 1 else np.zeros((student_num, assigned_group))
        for c in range(self.student_num):
            self.pr_assignment[c] = pr_assigner(c)

        self.build_eligibility_matrix(self.student_group_matrix, self.slot_list)
        R_samples = np.stack([self.eligibility_matrix] * self.r)
            ...
            if assigned_group == 1:
                ...
                for c, pr in enumerate(self.pr_assignment):
                    for g, g_slotlist in self.group_slot_idx_map.items(): 
                        ...
                        g_idx = (self.student_group_matrix[c].nonzero()[0] == g).nonzero()[0]
                        R_samples[i, c, g_slotlist] = np.random.binomial(n=1, p=pr[g_idx])

        R_train, R_test = sklearn.model_selection.train_test_split(R_samples, train_size=train_r)
        self.R_train_samples = R_train
        self.R_test_samples = R_test

exp = SyntheticExp(student_num, slot_list, total_group, assigned_group, magnitude, train_n, test_n)
